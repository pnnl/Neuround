...
Create a variable with optional type metadata.
...
**Returns:** Variable object. If type params are given, additional attributes are attached: `var_types`, `num_vars`, `integer_indices`, `binary_indices`, `continuous_indices`, `relaxed`.
...
| `constraints` | `list` | | `Constraint` objects |
...
- **`train(loader_train, loader_val, optimizer, epochs=200, patience=20, warmup=20, device="cpu")`** -- Train with early stopping.
